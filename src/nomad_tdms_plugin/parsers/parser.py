...
from .parse_tdms_helpers import INDEX_MAPPING, detect_cycles, extract_index_series_all, get_file_timerange, load_tdms_file, save_cycle, timed, filter_cycle
...
class NewParser(MatchingParser):
    def parse(
        ...
        index_df = timed("Index laden", extract_index_series_all, logger, tdms_file_paths)
        ...
        for i, (c, unvollst) in enumerate(cycles, start=1):
            ...
            if not marker:
                logger.info("NewSchema.parse", parameter=f" ⚠ Kein Mapping (TYP/TEMP/Zustand) gefunden, Zyklus übersprungen")
            ...
            # Filtern & Speichern
            cycle_data = timed(f"Filter Zyklus {i}", filter_cycle, logger, tdms_file_paths, c, file_ranges)
            timed(
                f"Speichern Zyklus {i}",
                save_cycle, logger, cycle_data, typ, temp, zustand, zyklus_counter[key], c, unvollst
            )
